 ...
 * Version: Build v0.0.4
 * This is not a stable version. You can check for details and get the latest
 * version at https://github.com/fei0319/poly.h If you have discovered any bugs,
 * please report about it at https://github.com/fei0319/poly.h/issues/new
 */
...
FLOAT_T sin(const FLOAT_T &x,
            const long double &EPS = fstdlib::_FEISTDLIB_SECURITY_::EPS) {
...
poly poly::prefix(std::size_t len) const {
    ...
    for (int i = std::min(len, size()) - 1; i >= 0; --i)
        f[i] = data[i];
    return f;
}
 ...
 * . Note that this implementation can perform correctly only when N is around
 * or less than 1e5.
 */
...
m_poly operator*(const m_poly &_f, const m_poly &_h) {
    ...
    for (int i = 0; i < (int)_f.size(); ++i)
        f[i].x = _f[i] & ((1 << 15) - 1), f[i].y = _f[i] >> 15;
    for (int i = 0; i < (int)_h.size(); ++i)
        h[i].x = _h[i] & ((1 << 15) - 1), h[i].y = _h[i] >> 15;
...
poly poly::inv(void) const {
    ...
    for (int w = 2; w / 2 < sz; w <<= 1) {
        ...
        for (int i = 0; i < w * 2; ++i)
            f[i] = (ll)f[i] * (2 - (ll)f[i] * g[i] % mod) % mod;
...
m_poly log(const m_poly &h) {
    ...
    d.resize(h.size());
    for (int i = d.size() - 1; i >= 1; --i)
        d[i] = (ll)d[i - 1] * _FEISTDLIB_SECURITY_::qpow(i, mod - 2, mod) % mod;
    d[0] = 0;
    return d;
}
